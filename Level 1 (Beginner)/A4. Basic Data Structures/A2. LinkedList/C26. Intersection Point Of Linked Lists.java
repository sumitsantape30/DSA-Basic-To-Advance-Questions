...
public class KthEleFromEnd {
	...
	public static class LinkedList {
		...
		public static int findIntersection(LinkedList one, LinkedList two){
			...
			Node t1= one.head; //one ke head pe ek ko bithaya
			Node t2= two.head; //two ke head pe ek bithaya
			
			int delta= Math.abs(one.size - two.size); //donoka difference nikalenge aur absolute value lenge hume wahi chahiye
			if( one.size > two.size ) {
				//agar one bda hai to t1 ko chalayenge coz woh intersection point se jada dur hai, woh utna dur hai jitna uska size jda hai
			...
			// jab t1 and t2 bahar ayenge dono intersection point se ek jitne duri pr honge
			while( t1 != t2) { //ab tabtak kam karenge jabtak t1 and t2 equal nhi hota
				//tabtak t1 t2 dono ek ek badhenge
				t1= t1.next;
				t2= t2.next;
			}
			
			//jab bahar ayenge to t1 t2 ekdum barabr honge
			return t1.data;
		}
