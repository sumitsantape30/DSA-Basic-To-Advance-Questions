...
public class Main {
	...
	public static int goldmine(int[][] arr) {
		...
		for (int j = arr[0].length - 1; j >= 0; j--) { //hum solve krte waqt column by column piche ki taraf aa rhe so column ko bahar rakhenge aur khudko(j) ko sabse last mai rakhenge
			for (int i = arr.length; i >= 0 ; i--) { //aag iss column mai niche se upar ya upar se niche travel karo faraq nhi pdta
				if (j == arr[0].length - 1) { //agar last column hai hai to dig nhi kr skta aage, jo arr[i][j] ai pda hoga wahi humara answer hojayega
					dp[i][j] = arr[i][j];
					
				} else if (i == 0) { //agar first row mai hai to hum 2 jagah ja skte hai 
					
					dp[i][j] = arr[i][j] + Math.max(dp[i][j + 1], dp[i + 1][j + 1]); //khudka gold collect kiya aur dono jagah ka max liya
