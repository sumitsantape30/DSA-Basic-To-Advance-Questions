...
public class Main {
  ...
  public static void transBackFromLeftClonedTree(Node node){
    ...
    transBackFromLeftClonedTree(node.left.left); // maine node ke left ke left ko bola trnafrom back hoke aajao
    transBackFromLeftClonedTree(node.right);
    
    //niche wala kam preorder mai karo or postorder mai karo
    node.left = node.left.left; // node ka left banega node ke left ka left
  }
