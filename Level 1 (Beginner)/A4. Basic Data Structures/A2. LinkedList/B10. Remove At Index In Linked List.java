...
class LinkedList {
	...
	public void removeAt( int idx) {
		if( idx < 0 || idx >= size) { //agar index negative ya fir >= size de diya hai to hum bolenge invalid arguments
			System.out.println("Invalid Arguments");
		}else if( idx == 0) { //agar index 0 pass kiya to mai sidha removeFirst() krdunga
			removeFirst();
		}else if( idx == size -1){ // index last rha to mai removeLast() ko bolunga remove karne
			removeLast();
		}else {
			//agar bich ka koi element remove karna hai to yeh mai handle karunga
			Node temp= head;
			for( int i=0; i< idx-1; i++) {
				temp= temp.next;
			}
			
			//mera temp ab uss index ke ek piche aagya hai
