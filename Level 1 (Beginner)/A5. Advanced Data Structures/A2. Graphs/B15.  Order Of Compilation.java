...
public class Main {
   ...
   public static void main(String[] args) throws Exception {
    ...
    //DFS se stack bharwake lani hai
    ...
    for (int i = 0; i< visited.length; i++) {
      if(visited[i] == false) { 
        dfs(graph, i, visited, st); // is function se puri stack bharke ajayegi
      }
    }

    while (st.size() > 0) { //jabtak stack khali nhi hota remove karke print karte rahenge
  ...
  public static void dfs(ArrayList<Edge>[] graph, int src, boolean[] visited, Stack<Integer> st) {
    ...
    //aatehi visited mark krdo
    visited[src] = true;
    for (Edge e : graph[src]) {
        int nbr= e.nbr;
        if (visited[nbr] == false) { //agar woh unvisited nbr hai to usko call lagni chahiye
          dfs(graph, nbr, visited, st);
      }
    }
    //wapas jane se pehle usko stack mai push krdo
    st.push(src);
  }
}
